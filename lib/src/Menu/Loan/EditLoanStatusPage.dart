  ...
  Future<void> fetchLoanRequests() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("${config.apiUrl}/loan-requests"),
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("อัปเดตเรียบร้อย")));
      } else {
    ...
    return Scaffold(
      ...
      body:
          _isLoading
              ...
              ? const Center(
                child: Text("ไม่มีคำขอกู้ยืม", style: TextStyle(fontSize: 18)),
              )
              : ListView.builder(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            "จำนวนเงินที่ขอกู้: ฿${item['loan_amount']}",
                            style: const TextStyle(),
                          ),
                            ...
                            TextButton.icon(
                              ...
                              label: Text(
                                loanRequests[index]['due_date'] != null
                                    ? loanRequests[index]['due_date'].substring(
                                      0,
                                      10,
                                    )
                                    : "เลือกวันครบกำหนด",
                                ...
                                showDialog(
                                  ...
                                  builder:
                                      (_) => AlertDialog(
                                        ...
                                        content: const Text(
                                          "คุณต้องการบันทึกสถานะนี้ใช่หรือไม่?",
                                        ),
                                        actions: [
                                          TextButton(
                                            onPressed:
